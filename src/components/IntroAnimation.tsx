...
export function IntroAnimation({ onComplete }: Props) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 9999, overflow: 'hidden' }}>
      ...
      {/* ─── LOGO (revealed by planer clip-path, retracts to header) ─── */}
      <div style={{
        ...
        <motion.img
          ...
          style={{ filter: 'brightness(1.3) drop-shadow(0 0 30px rgba(180,140,60,0.3))' }}
          initial={{ clipPath: 'inset(0 102% 0 0)', width: 'min(380px, 55vw)' }}
          animate={
            isOpening
              ? {
                  ...
                  y: `calc(-50vh + 52px)`,
                  width: '120px',
                  filter: 'brightness(1) drop-shadow(0 0 0px rgba(180,140,60,0))',
                }
              : isPlaning || stage === 'pausing'
                ? { clipPath: 'inset(0 0% 0 0)', x: 0, y: 0, width: 'min(380px, 55vw)' }
                : { clipPath: 'inset(0 102% 0 0)', x: 0, y: 0, width: 'min(380px, 55vw)' }
          }
          transition={
            isOpening
              ? { duration: DOOR_DURATION * 0.85, ease: [0.76, 0, 0.24, 1] }
              : isPlaning
                ? { duration: PLANER_DURATION, ease: 'linear' }
                : { duration: 0 }
          }
        />
      </div>
